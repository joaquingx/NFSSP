...
Schedule randomPermutation::getRandomPermutation() {
    ...
    for(int i = 0 ; i < nJobs; ++i){
        resultSchedule->addPseudoJob(0,nMachines,toShuffle[i]);
    }
...
LR::LR(int nMachine, int nJobs, const Matrix &instance) : cHeuristic(nMachine, nJobs, instance) {
...
cHeuristic::cHeuristic(int nMachines, int nJobs, const Matrix &instance) {
    this->nMachines = nMachines;
    this->nJobs = nJobs;
    this->instance = instance;
    resultSchedule = new Schedule(nMachines,nJobs,instance);
}
